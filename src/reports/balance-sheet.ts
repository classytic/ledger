...
export async function generateBalanceSheet(
  ...
  const {
    ...
    retainedEarningsDisplayCode = country.retainedEarningsDisplayCode ?? retainedEarningsAccountCode,
  ...
  const reAccountBalance = reAccountResults.length > 0 ? reAccountResults[0].c - reAccountResults[0].d : 0;
  ...
  const reGroup: ReportGroup = {
    ...
    accounts: [
      { id: 'prior-retained', name: 'Previous Years Retained Earnings', code: retainedEarningsDisplayCode ?? retainedEarningsAccountCode ?? '', balance: priorRetained },
